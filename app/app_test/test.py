...
from vk_python import Ui_vk
from main import Ui_Main
from inst import Ui_Inst
from dialog import Ui_Login
...
def data():
    global login, passwd, ui_log, auth
    login = ui_log.dialog_login.text
    passwd = ui_log.dialog_password.text
    auth.close()
...
def sniffer_with_login(driver,url,login,passwd):
    photo_btn = driver.find_elements_by_class_name("page_counter")
    k = 0
    for i in range(len(photo_btn)):
        # print(photo_btn[i].text)
    ...
    if body.find("http://instagram.com") != -1:
        ...
        url_end = body.find("\"",url_start)
        print(body[url_start:url_end])
        sniffer_inst_in_vk(body[url_start:url_end],name_dir[1])

    if not name_dir[1] in os.listdir():
        os.mkdir(name_dir[1])
    if find_css(driver, "#profile_photos_module > a.module_header > div"):
        driver.find_element_by_css_selector("#profile_photos_module > a.module_header > div").click()
        time.sleep(2)
    ...
    k = 0
    photos = webdriver.Chrome()
    photos.get("https://vk.com")
    login_input = photos.find_element_by_css_selector("#index_email")
    password_input = photos.find_element_by_css_selector("#index_pass")
    login_input.send_keys(login)
    password_input.send_keys(passwd)
    photos.find_element_by_css_selector("#index_login_button").click()
    time.sleep(3)
    # print(find_css(driver, "#login_message > div"))
    if find_css(driver, "#login_message > div"):
        return False
    for i in years:
        id_photos = i.get_attribute("data-id")
    ...
    predict = []
    detector = dlib.get_frontal_face_detector()
    for i in os.listdir(name_dir[1]):
        im = io.imread(name_dir[1]+ "\\" + i)            
        dets = detector(im,1)
        print(dets)
        if not dets:
            os.remove(name_dir[1] + "\\" + i)
            continue
        img = np.array(Image.open(name_dir[1]+ "\\" +i))/255.0
        img = img.reshape(1,img.shape[0],img.shape[1],img.shape[2])
        predict.append(get_string(np.argmax(model.predict(img))))
    #print(predict)
    if 'cadet' in predict:
...
def sniffer_with_api(vk, url, id):
    
    driver = webdriver.Chrome()
    driver.get(url)
    time.sleep(1)
    if test_for_working_api(driver,url) == False:
        driver.close()
        return False
    time.sleep(1)
    #for i in range(1,15):
       # id = driver.find_element_by_xpath("/html/head/link[{}]").format(str(i)).get_attribute("href")
        #if re.search("=",id):
           # id = id.split("=")
            #id = int(id[1])
           # break
...
def group(driver, url):
    driver.get(url)
    time.sleep(1)
    if test_for_working(driver, url) == False:
       driver.close()
       return 0
    if find_css(driver, "#profile_idols > a > div") == False:
        driver.close()
        return 0
...
def log_in ():
    login = ui_vk.email.text()
    password = ui_vk.password.text()
    url = "https://vk.com/"+ui_vk.id_with_login.text()
    driver = webdriver.Chrome()
    driver.get("https://vk.com")
    if find_name(driver, "pass") == False:
        print("Вы уже авторизированны на сайте\n Выхожу из аккаунта...")
        driver.find_element_by_name("#top_profile_link").click()
        driver.find_element_by_id("top_logout_link").click()
    login_input = driver.find_element_by_css_selector("#index_email")
    password_input = driver.find_element_by_css_selector("#index_pass")
    login_input.send_keys(login)
    password_input.send_keys(password)
    driver.find_element_by_css_selector("#index_login_button").click()
    time.sleep(3)
    if find_css(driver, "#login_message > div"):
        ui_vk.error.setText("Неверный логин или пароль! Повторите попытку")
        driver.close()
        return False
    driver.get(url)
    time.sleep(1)
    if test_for_working(driver, url) == False:
       return False
    photo = sniffer_with_login(driver,url,login,password)
    ...
    if photo == 0 and social == 0:
       ...
       msg.setText("Военнослужащий или связанн с военной службой ")
    elif photo == 1 and social == 0:
       msg.setText("Военнослужащий или связаен с военной службой ")
    elif photo == 1 and social == 1:
       msg.setText("Военнослужащий")
    msg.exec()
    return True
...
def prove():
    url = "https://vk.com/" + ui_vk.id.text()
    photo =sniffer(url)     #photo = sniffer_with_api(url)
    driver = webdriver.Chrome()
    social = group(driver, url)
    #Icon = QtGui.QIcon("vk_text.png")
    msg = QMessageBox()
    msg.setWindowTitle("Результат")
    #msg.setIconPixmap(QtGui.QPixmap("vk_test.ico"))
...
def api():
    ...
    vk_session = vk_api.VkApi(token=token,api_version=vk_api.__version__,app_id=VERSION_ID)
    vk = vk_session.get_api()
    start = int(ui_vk.start.text())
    finish = int(ui_vk.finish.text())
    for i in range(start,finish):
        url = "https://vk.com/id"+str(i)
        photo = sniffer_with_api(vk, url,i)    #photo = sniffer(driver,url)
        driver = webdriver.Chrome()
        social = group(driver, url)
        msg = QMessageBox()
        msg.setWindowTitle("Результат") 
...
def vk():
    global vka, ui_vk
    vka = QtWidgets.QWidget()
    ui_vk = Ui_vk()
    ui_vk.setupUi(vka)
    vka.setWindowIcon(QtGui.QIcon("vk_test.png"))
    Dialog.close()
    vka.show()
    ui_vk.proved_with_login.clicked.connect(log_in)
    ui_vk.proved.clicked.connect(prove)
    ui_vk.proved_with_api.clicked.connect(api)

def inst():
    global insta,ui_insta
    insta = QtWidgets.QWidget()
    ui_insta = Ui_Inst()
    ui_insta.setupUi(insta)
    insta.setWindowIcon(QtGui.QIcon("inst.png"))
    Dialog.close()
    insta.show()
    ui_insta.analiz.clicked.connect(sniffer_inst)
...
if __name__ == '__main__':
    ...
    Dialog.setWindowIcon(QtGui.QIcon("main.jpg"))
    ui = Ui_Main()
    ui.setupUi(Dialog)
    Dialog.show()
    ui.pb_vk.clicked.connect(vk)
    ui.pb_inst.clicked.connect(inst)
    sys.exit(app.exec_())
